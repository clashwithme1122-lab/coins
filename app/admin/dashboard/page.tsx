...
import { Plus, Edit2, Trash2, Save, X, LogOut, Coins, Eye, Check, XCircle, RotateCcw, Gavel, Clock, TrendingUp, Users, MessageCircle, Truck, Package, UserCheck, User } from 'lucide-react'
...
export default function AdminDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<'coins' | 'submissions' | 'appraisals' | 'auctions' | 'users' | 'tracking' | 'chat'>('coins')
            ...
            messages: [
              {
                ...
                userName: 'John Smith',
                adminId: 'admin',
                message: 'Hi, I won the Morgan Dollar auction. When will it be shipped?',
                sender: 'user',
                timestamp: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(),
                read: false
              }
            ],
            lastMessage: 'Hi, I won the Morgan Dollar auction. When will it be shipped?',
  ...
  const addTrackingNumber = () => {
    if (!trackingForm.userId || !trackingForm.auctionId || !trackingForm.trackingNumber) {
      showError('Please fill all tracking fields')
      return
    }

    const user = users.find(u => u.id === trackingForm.userId)
    const auction = auctions.find(a => a.id.toString() === trackingForm.auctionId)

    if (!user || !auction) {
      showError('Invalid user or auction')
      return
    }

    const newTracking: TrackingOrder = {
      id: Date.now().toString(),
      userId: trackingForm.userId,
      userName: user.name,
      auctionId: trackingForm.auctionId,
    ...
    const updatedOrders = [...trackingOrders, newTracking]
    setTrackingOrders(updatedOrders)
    localStorage.setItem('trackingOrders', JSON.stringify(updatedOrders))

    showSuccess(`Tracking number added for ${user.name}`)
    setTrackingForm({ userId: '', auctionId: '', trackingNumber: '' })
  }

  const updateTrackingStatus = (orderId: string, status: TrackingOrder['status']) => {
        ...
        <div className="mb-8">
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              ...
              >
                Chat ({chats.reduce((sum, chat) => sum + chat.unreadCount, 0)} unread)
              </button>
            </nav>
          </div>
        </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {users.map((user) => (
                      <tr key={user.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-900">Spent: ${user.totalSpent.toLocaleString()}</div>
                          <div className="text-sm text-gray-500">Won: {user.wonAuctions} • Active: {user.activeBids}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex space-x-2">
                            <button
                            ...
                            <button
                              ...
                              title="Send Message"
                              onClick={() => {
                                const userChat = chats.find(c => c.userId === user.id)
                                if (userChat) {
                                  setSelectedChat(userChat)
                                  setActiveTab('chat')
                                } else {
                                  showInfo('No chat history with this user')
                                }
                              }}
                            >
        ...
        {activeTab === 'tracking' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-xl font-semibold mb-6">Add Tracking Number</h2>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">User</label>
                  <select
                    value={trackingForm.userId}
                    onChange={(e) => setTrackingForm({ ...trackingForm, userId: e.target.value })}
                ...
                <div>
                  ...
                  <select
                    value={trackingForm.auctionId}
                    onChange={(e) => setTrackingForm({ ...trackingForm, auctionId: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-purple-600"
                  >
                    <option value="">Select Auction</option>
                    {auctions.map(auction => (
                      <option key={auction.id} value={auction.id.toString()}>{auction.title}</option>
                    ))}
                  </select>
                </div>
                <div>
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-xl font-semibold">Tracking Orders ({trackingOrders.length})</h2>
              </div>

              {trackingOrders.length === 0 ? (
                <div className="p-12 text-center">
                  <Truck className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                  <p className="text-gray-500">No tracking orders yet</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {trackingOrders.map((order) => (
                        <tr key={order.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-900 max-w-xs truncate">{order.auctionTitle}</div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4">
                            <select
                              value={order.status}
                              onChange={(e) => updateTrackingStatus(order.id, e.target.value as TrackingOrder['status'])}
                          ...
                          <td className="px-6 py-4">
                            <div className="flex space-x-2">
                              <button
                                className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                                title="Track Package"
                                onClick={() => window.open(`https://www.fedex.com/fedextrack/?trknbr=${order.trackingNumber}`, '_blank')}
                              >
